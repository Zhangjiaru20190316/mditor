...
- **云同步（v4.12，可选）**：自带存储（BYO storage）双向同步工作区目录到你自己的 S3 兼容对象存储（七牛云 Kodo / 阿里云 OSS / Cloudflare R2 / MinIO 自建 / AWS S3 / 任意自定义端点）。默认关闭、不开即零网络；文件级 3-way 合并，冲突自动产生 `.冲突-时间戳` 副本双端可见，本地删除进应用内回收站；支持保存后防抖 / 定时 / 启动自动同步与断网自动恢复；状态栏四态指示器，多窗口状态一致。鸿蒙端 v4.13 起同步支持（已真机验收）。**跨设备配对**：对象键按「工作区文件夹名」分前缀——两台设备把**同名文件夹**各自挂为工作区（如 PC 的 `E:\笔记` ↔ 平板的「笔记」目录）即可双向同步；不同名的工作区互不相通（各自独立备份）。
