...
### 模块 1：全库索引 + 快速切换器（随 4.7 前期 commit 交付）

- workspace 全量 .md 轻量行扫描索引（标题/大纲/双链/标签/闪卡/mtime，不落盘），分批 + idle 调度，保存与文件监听走单文件增量；**Ctrl+P** 模糊跳转（频次/最近加权）

### 模块 2：双向链接 + 反链面板 + 标签（随 4.7 前期 commit 交付）

- `[[目标]]` / `[[目标|显示]]` 语法 + 输入补全 + 点击跳转（重名路径消歧）；侧栏「链接」页反链（含上下文片段）与全库标签过滤；导出降级为标准链接/纯文本

### 模块 3：学术引用链 + LaTeX 导出

- 自研 BibTeX 解析器（Zotero Better BibTeX 兼容，坏条目跳过告警不中断；200+ 条目验收）；`[@citekey]` / `[@key, p. 12]` / 多键行内引用，选区工具栏「@」引用选择器
- `# References` / `# 参考文献` 标题下自动生成文献表（编辑器内只读 widget + 静态渲染 + 导出三处共用同一份格式化纯函数）；numeric 引用序 / author-year 字母序（APA 简化）两样式
- 图表编号交叉引用：`![caption](x.png){#fig:id}`、`: caption {#tbl:id}`、正文 `@fig:id` → 「图 N」（照抄公式编号管线模式，静态渲染与导出生效）
- **导出 LaTeX (.tex)**：纯前端 md→tex（不调 pandoc、不加 shell 权限）——ctexart/XeLaTeX、公式天然平移、`\cite` + thebibliography、figure/tabular（booktabs）环境、双链/HTML 降级

### 模块 4：间隔重复闪卡

- `:::flash` 问答卡容器（编辑器内卡片样式块、内容可直接编辑；空段输入 `:::flash` 即时成卡）；SM-2 简化调度（间隔表 0/1/3/7/14/30 天 × ease 因子 2.5±0.15 下限 1.3；忘了/困难/良好/轻松四档）
- 做卡入口：选区工具栏「卡」（答案预填选区）+ AI 菜单「改写为问答卡」（AI 生成 Q/A 人工确认）；复习模式（翻面 + 四档自评 + 逾期/需重定位提示 + 来源跳转）；进度存 `appDataDir/review-state.json`（损坏静默重建）
- 导出降级为普通引用块；到期扫描复用全库索引（保存增量更新）

### 模块 5：库级 AI 问答（RAG，默认关）

- Rust 新增 `ai_embed` 命令代理 OpenAI 兼容 `/embeddings`（渲染层不直连外网，CSP 不变）；嵌入服务配置独立于对话模型
- 向量索引 `appDataDir/rag-index.json`：按标题分节 ~1200 字符/块（重叠 1 句）、分批嵌入可暂停/续跑（暂停即落盘）、两层增量去重（mtime 门 + 块 id+hash 双校验）、换模型自动重建、big 模式让路
- AI 面板「全库问答」：首次构建成本确认（明示处理 N 个文档）→ 问题嵌入 → 余弦 top-8 → 流式回答 + 可点击来源列表（文件名 › 标题，直达对应笔记对应标题）；检索失败静默回退普通对话

### 工程纪律

- remark 插件计数哨兵同步：双链/引用/闪卡三插件两档无条件注册，小文档 8→11、大文档 5→8，锚定测试随行
- 全量 vitest 532 绿（新增 7 文件 132 用例：bib 解析容错/引用格式化一致性/图表编号/LaTeX 转换/闪卡调度与持久化/RAG 分块检索与嵌入 mock）
- 大文档性能模式兼容：索引扫描与嵌入批次让路，普通文档编辑路径零新增开销
...
- **AI 流式「停止」真正停拉上游流（计费根修）**：此前点「停止」只摘前端监听，Rust 侧继续拉上游流到自然结束——被中止请求的 token 照常消耗（旧轮已知问题实锤项）。新增 `ai_chat_cancel` 命令 + 取消注册表（流退出自动清理条目），流循环逐 chunk 检查、取消即停；前端 cancel() 通知后端（旧后端静默降级）
- **ai_chat 解析失败不再整段回显上游响应**：经 300 字符截断（与其它错误分支同款纪律）
- **列表互转用例的 TextSelection 告警伪象清除**：测试光标位 2→3（落在文本位）——此前每次 `npm test` 打印的 `(list_item)` stderr 来自测试自身的选区构造，非产品行为；噪声消失后真实告警不再被淹没
- 结案两项（证据在报告）：文档内搜索计数「0」为测量竞态（复验 267 个正常）；html-to-docx 无 src 崩溃在真实导出路径不可达（milkdown image src 恒有默认值）
...
- **大文档视口渲染（设置「大文档视口渲染」，默认关）**：与「大文档性能模式」的减配解耦——文档超过 3000 行或 500KB 时只启用 content-visibility 视口化（视口外内容跳过布局与绘制，含高度记忆与冷启动预热），**保留代码高亮与公式渲染**。224KB KaTeX 习题集实测：拖选 816~1832ms→40ms、三击选段 ~1500ms→64~96ms、点击公式 968~992ms→24~32ms；Ctrl+A 跨视口全选实测正常，查找/批注/大纲跳转与性能模式共用同一套视口化机制。适合公式/代码密集的大文档
- **选区工具栏新增「链接」与「fx 行内公式」按钮**（接替 crepe 内置工具栏的独有功能）：链接为 URL 弹层（留空应用＝去除链接）；行内公式把选区文字包成 `$…$`、选中公式时还原为普通文字。源码模式分别以 `[](…)` / `$` 包裹

### 性能

- **停用 crepe 内置 Toolbar**（功能已并入自研选区工具栏）：它对每个编辑器事务做选区序列化 + Vue 工具栏重渲染 + 浮层定位（2 次强制布局读），是三击选段卡顿的直接贡献者之一
- **拖选期间选区不再被反复序列化**：选区工具栏改为「拖选中只做折叠/包含性判断，松开（mouseup/keyup）才取一次文本与定位」，取文本走 PM 文本层绕开公式双份 DOM；状态栏「已选 N 字」改为尾随防抖（选区静止后读一次）——原先拖选每帧全选区序列化 + 每 150ms 第二遍统计
- **行内公式点击的内部编辑器泄漏根修**（patch）：公式提示层每次展示都新建整套 PM 内层编辑器且不销毁旧的——改为 Schema 单例、新建前销毁旧视图、同一公式重复点击直接复用
- **停键后的全文序列化挪到空闲窗口**（patch）：225KB 文档实测单次 80ms 的「比较+序列化」不再占用停笔后的滚动/点击帧（markdown 回调至多延后 ~0.8s，下游本就防抖）；附带可开关计时打点（`window.__MDITOR_PROFILE_SERIALIZE`）
- **代码块离视口拆除修复**（patch）：聚焦/选中状态下离视口的代码块与公式块原先永不拆除（拆除定时器被消费后不再重排），现在会重新排队、失焦后照常拆除

### 修复

- **MD-4011 误报（诊断口径）**：「DOM 节点持续增长」的 15 分钟趋势窗口不感知文档切换——切到大文档再切回来会被报成每分钟上万节点的持续增长（实测 14.5 分钟连报 24 条，均无真实泄漏）。现在按文档标识切割趋势窗口，只统计同一文档内的趋势
- 打开大文档时若档位判定翻转（如开启视口渲染后首次打开），内容不再可能丢失（空编辑器）
...
- **文件外部修改监听自始失效（MD-6004「Command watch not found」）**：`tauri-plugin-fs` 依赖一直未启用 `watch` cargo 特性，Rust 侧从未注册 watch 命令，「外部修改自动同步」在全部历史构建中静默失效（capabilities 早已放行 `fs:allow-watch`，缺的只是特性开关）。v4.3.0 的诊断埋点把它暴露为 MD-6004 告警卡。修复：`Cargo.toml` 改为 `tauri-plugin-fs = { version = "2", features = ["watch"] }`（Cargo.lock 随之引入 notify）。判据：打开文件后 dev-events 心跳 `watch:attach` 计数存在且 `file:watch-fail` 不再增长
...
- 待手验清单（GUI）：
  ...
  7. 打开一个 .md 后用外部编辑器改动保存 → 编辑器自动同步（无「文件监听启动失败」告警卡）
