...
describe("remarkPipeline（worker 侧解析一致性）", () => {
  ...
  it("[@引用] → citation 节点（v4.7 模块 3 remarkCitation，native 模式）", () => {
    const tree = parseMarkdownTree(proc, "引用 [@smith2020] 与 [@lee2021, p. 12]。");
    const cites: N[] = [];
    const walk = (n: N) => {
      if (n.type === "citation") cites.push(n);
      (n.children ?? []).forEach(walk);
    };
    walk(tree);
    ...
    // big 档（无 math）同样产出 citation——插件无条件注册。
    const treeNoMath = parseMarkdownTree(procNoMath, "见 [@k]。");
    const kinds: string[] = [];
    const walk2 = (n: N) => {
      kinds.push(n.type ?? "?");
      (n.children ?? []).forEach(walk2);
